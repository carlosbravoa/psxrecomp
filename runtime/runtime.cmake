...
set(PSXRECOMP_RUNTIME_SOURCES
    ...
    ${PSXRECOMP_ROOT}/runtime/src/vram_upload_log.c
    ...
    ${PSXRECOMP_ROOT}/runtime/src/texture_pack.c
    ${PSXRECOMP_ROOT}/runtime/src/fmv_pack.c
    ${PSXRECOMP_ROOT}/runtime/src/png_read.c
    ...
    ${PSXRECOMP_ROOT}/runtime/src/disc_tree.cpp
...
function(psxrecomp_add_runtime_target target)
    ...
    # psx-disc-tree: CLI twin of the runtime's disc-tree engine (layout /
    # verify / build / md5 of an extracted disc tree — docs/DISC_TREE.md).
    # One per build tree; every game project gets it next to its runtime.
    if(NOT TARGET psx-disc-tree)
        add_executable(psx-disc-tree
            ${PSXRECOMP_ROOT}/runtime/tools/disc_tree_cli.cpp
            ${PSXRECOMP_ROOT}/runtime/src/disc_tree.cpp
            ${PSXRECOMP_ROOT}/runtime/src/iso_reader.cpp
            ${PSXRECOMP_ROOT}/runtime/src/cue_sheet.cpp)
        target_include_directories(psx-disc-tree PRIVATE ${PSXRECOMP_RUNTIME_INCLUDE_DIRS})
        target_link_libraries(psx-disc-tree PRIVATE chdr-static)
        set_target_properties(psx-disc-tree PROPERTIES CXX_STANDARD 17 CXX_STANDARD_REQUIRED ON)
    endif()
    ...
    # A build tree with several runtime targets (psx-runtime + psx-oracle under
    # BUILD_TESTING) would GENERATE the same stamp twice on single-config
    # generators, which CMake rejects; stamp once per build tree.
    get_property(_psxrt_ver_stamped GLOBAL PROPERTY PSXRT_GAME_VERSION_STAMPED)
    if(NOT _psxrt_ver_stamped)
        set_property(GLOBAL PROPERTY PSXRT_GAME_VERSION_STAMPED ON)
        file(GENERATE
