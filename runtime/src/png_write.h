...
/* Write an RGB (3 bytes/pixel) or RGBA (4 bytes/pixel) top-down buffer as a
 * PNG (colour type 2 / 6). Returns 1 on success. */
static int png_write_channels(FILE *f, const uint8_t *pix, uint32_t w, uint32_t h, int channels) {
    ...
    ihdr[8]=8;   /* bit depth   */
    ihdr[9]=(uint8_t)(channels == 4 ? 6 : 2);   /* 2 = truecolor RGB, 6 = RGBA */
    ...
    const size_t bpp = (size_t)channels;
    size_t raw_len = (size_t)h * (1 + (size_t)w * bpp);
    uint8_t *raw = (uint8_t *)malloc(raw_len);
    if (!raw) return 0;
    for (uint32_t y = 0; y < h; y++) {
        uint8_t *row = raw + (size_t)y * (1 + (size_t)w * bpp);
        row[0] = 0;
        memcpy(row + 1, pix + (size_t)y * w * bpp, (size_t)w * bpp);
    }
...
#define PNG_WRITE_MAYBE_UNUSED __attribute__((unused))
#else
#define PNG_WRITE_MAYBE_UNUSED
#endif
PNG_WRITE_MAYBE_UNUSED static int png_write_rgb(FILE *f, const uint8_t *rgb, uint32_t w, uint32_t h) {
    return png_write_channels(f, rgb, w, h, 3);
}
PNG_WRITE_MAYBE_UNUSED static int png_write_rgba(FILE *f, const uint8_t *rgba, uint32_t w, uint32_t h) {
    return png_write_channels(f, rgba, w, h, 4);
}
